 ...
 2) Core components & representative files:
    ...
    - Evaluation: `evaluation/` (metrics, evaluators, backend_dashboard for assessing RAG performance)
 ...
 3) Essential developer workflows (bash/Linux):
    ...
    - Run ground truth evaluation: `python run_ground_truth_evaluation.py`
 ...
 5) Integration points & gotchas:
    ...
    - LLM providers: Gemini requires `GOOGLE_API_KEY`, LMStudio local server, Ollama for local models.
 ...
 6) Quick search & rerank trace (where to look when debugging):
    ...
    - Evaluation → `evaluation/backend_dashboard/api.py` (recall, relevance, semantic similarity)
