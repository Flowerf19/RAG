...
from RAG_system.LOADER.normalizers.table_utils import filter_duplicate_tables
...
class PDFLoader:
    ...
    def __init__(
        ...
        # If True, call PDFDocument.normalize() on the returned document
        # This performs page/block/table-level normalization and filtering
    ...
    def _validate_config(self) -> None:
        ...
        if self.tables_engine not in ('auto', 'camelot', 'pdfplumber'):
            logger.warning(
                "Unknown tables_engine '%s', falling back to 'auto'", self.tables_engine)
    ...
    def create_default(cls) -> 'PDFLoader':
        ...
        return cls(
            ...
            enable_sentence_segmentation=True, auto_normalize=True
        )
                ...
                for t in table_objs:
                    ...
                    if not table_title_candidates:
                        ...
                        if fallback_candidates:
                            below = [
                                c for c in fallback_candidates if c['position'] == 'below']
                            above = [
                                c for c in fallback_candidates if c['position'] == 'above']

                            if below:
                                below.sort(key=lambda x: (
                                    x['dist_y'], -x['font_size']))
                                caption = below[0]['text']
                            elif above:
                                above.sort(key=lambda x: (
                                    x['dist_y'], -x['font_size']))
                    ...
                    best_candidate = None
                    if below_candidates:
                        below_candidates.sort(key=lambda x: (
                            x['dist_y'], x['dist_x'], -x['font_size']))
                        best_candidate = below_candidates[0]
                    elif above_candidates:
                        above_candidates.sort(key=lambda x: (
                            x['dist_y'], x['dist_x'], -x['font_size']))
    ...
    def load_pdf(self, file_path: str) -> PDFDocument:
        ...
        doc_title, page_labels, meta, num_pages = PDFDocument.extract_metadata(
            file_path)
        ...
        for page_idx in range(doc.page_count):
            ...
            original_blocks = all_blocks[page_idx] if all_blocks and page_idx < len(
                all_blocks) else []
                    ...
                    for block in blocks:
                        ...
                        if self.enable_sentence_segmentation:
                            try:
                                ...
                                logger.warning(
                                    f"Block normalization failed on page {page_idx+1}: {e}")
            ...
            if self.extract_tables:
                try:
                    raw_tables = self._extract_tables_for_page(
                        file_path, page_idx + 1, plumber_pdf)
                    ...
                    for raw_table in raw_tables:
                        matrix = raw_table.get('matrix', raw_table) if isinstance(
                            raw_table, dict) else raw_table
                        bbox = raw_table.get('bbox') if isinstance(
                            raw_table, dict) else None

                        # Clean individual table
                        cleaned_matrix = clean_table(matrix)
                        if cleaned_matrix:  # Only keep if not filtered out
                            tables.append(
                                {'matrix': cleaned_matrix, 'bbox': bbox})

                    # Also filter duplicates across all tables
                    if len(tables) > 1:
                        matrices_only = [t['matrix'] for t in tables]
                        unique_matrices = filter_duplicate_tables(
                            matrices_only)
                    ...
                    logger.warning(
                        f"Table extraction failed for page {page_idx+1}: {e}")
            ...
            if tables:
                ...
                if page_idx + 1 < doc.page_count:
                    next_page_blocks = all_blocks[page_idx + 1] if all_blocks and page_idx + 1 < len(
                        all_blocks) else None
                ...
                for table_dict in tables:
                    ...
                    if matrix:
                        ...
                        table_obj = TableSchema.from_matrix(
                            matrix, file_path=file_path, page_number=page_idx + 1, bbox=bbox)

                        # Try to extract table caption using ORIGINAL blocks (before merging)
                        # Also check next page if table is at bottom
                        caption = self._extract_table_caption(
                            page, bbox, original_blocks, next_page_blocks, page_height)
                        if caption:
                            if table_obj.metadata is None:
                                logger.debug(
                                    f"Page {page_idx+1}: metadata was None, initializing")
                                table_obj.metadata = {}
                            table_obj.metadata['table_caption'] = caption
                            logger.debug(
                                f"Extracted caption for table on page {page_idx+1}: {caption}")
                            logger.debug(
                                f"  Metadata after assignment: {table_obj.metadata}")

                        # Create TableBlock with text representation
                        # Generate text from table for chunking/filtering
                        table_text_lines = []
                        if table_obj.header:
                            table_text_lines.append(" | ".join(
                                str(h) for h in table_obj.header))
                        for row in table_obj.rows:
                            if hasattr(row, "cells"):
                                row_text = " | ".join(str(c.value)
                                                      for c in row.cells)
        ...
        # Optionally run full document normalization (page/block/table-level)
        if getattr(self, 'auto_normalize', False):
            try:
                document.normalize()
            except Exception as e:
                logger.warning(
                    f"Document normalization failed for {file_path}: {e}")
