...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader(
        "Chọn file để tải lên", type=["pdf", "docx", "txt"]
    )
...
if sources:
    ...
    st.info(
        "Chưa có nguồn tham khảo nào được tìm thấy. Hãy đặt câu hỏi để hệ thống tìm kiếm tài liệu liên quan."
    )
...
def ask_backend(prompt_text: str) -> str:
    ...
    try:
        ...
        try:
            ret = fetch_retrieval(
                prompt_text, top_k=10, max_chars=8000
            )  # Tăng lên 8000
        ...
        messages = build_messages(
            query=prompt_text, context=context, history=st.session_state["messages"]
        )
...
prompt = st.chat_input(
    "Type a new message here", disabled=st.session_state.get("is_generating", False)
)
